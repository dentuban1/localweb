...
<section x-data="window.bitflan.components.dns_lookup_component()" x-init="init(<?php echo alpinify($js_errors); ?>)" @keyup.enter="submit()">
	<div class="searchSection mainPadding">
		...
		<button x-on:click="submit()" class="btn searchButton" type="submit" value="submit">
			...
			<span x-cloak x-show="sending"><img src="<?php echo $this->theme->url('assets/images/search_loader.svg'); ?>" alt="<?php echo lang('gen_search_loader'); ?>" title="<?php echo lang('gen_search_loader'); ?>"/></span>
		</button>
	</div>
		...
		<div class="result-content-inner">
			<div class="result-main-title clearfix">
				<div class="left-title-area plus"><span><svg xmlns="http://www.w3.org/2000/svg" width="17.812" height="17.812" viewBox="0 0 54.194 54.194">
						...
						</svg></span><?php echo lang('domain_dns'); ?></div>
			</div>
						...
						<template x-if="data">
							<table class="table table-striped table-bordered bg-light mb-0">
								<thead class="bg-primary text-light">
									<tr>
										<th class="text-center"><?php echo lang('dns_records'); ?></th>
										<th class="text-center"><?php echo lang('dns_ttl'); ?></th>
										<th class="text-center"><?php echo lang('dns_class'); ?></th>
										<th><?php echo lang('dns_entries_for'); ?> <span x-init="$el.textContent = domain"></span></th>
									</tr>
								</thead>
								<tbody>
									<template x-if="data.a.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-primary"><?php echo lang('dns_a'); ?></span></h4>
											</td>
									...
									<template x-if="data.aaaa.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-info"><?php echo lang('dns_aaaa'); ?></span></h4>
											</td>
									...
									<template x-if="data.ns.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-success"><?php echo lang('dns_ns'); ?></span></h4>
											</td>
									...
									<template x-if="data.mx.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-danger"><?php echo lang('dns_mx'); ?></span></h4>
											</td>
									...
									<template x-if="data.soa.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-warning"><?php echo lang('dns_soa'); ?></span></h4>
											</td>

											<td class="align-middle text-center" x-text="data.soa.ttl"></td>
											<td class="align-middle text-center" x-text="data.soa.class"></td>
											<td>
												<h5 class="text-dark"><?php echo lang('dns_email'); ?>: <span x-text="data.soa.data.email"></span></h5>
												<h5 class="text-dark"><?php echo lang('dns_serial'); ?>: <span x-text="data.soa.data.serial"></span></h5>
												<h5 class="text-dark"><?php echo lang('dns_refresh'); ?>: <span x-text="data.soa.data.refresh"></span></h5>
												<h5 class="text-dark"><?php echo lang('dns_retry'); ?>: <span x-text="data.soa.data.retry"></span></h5>
												<h5 class="text-dark"><?php echo lang('dns_expire'); ?>: <span x-text="data.soa.data.expire"></span></h5>
												<h5 class="text-dark"><?php echo lang('dns_minimum_ttl'); ?>: <span x-text="data.soa.data.minimum_ttl"></span></h5>
											</td>
										</tr>
									</template>
									...
									<template x-if="data.txt.data">
										<tr>
											<td class="align-middle text-center">
												<h4><span class="badge badge-secondary"><?php echo lang('dns_txt'); ?></span></h4>
											</td>
...
<section>
<?php
$this->theme->view('components/dns_features');
$this->load->view('core/middle_ad_spot');
$this->theme->view('components/dns_faq');
